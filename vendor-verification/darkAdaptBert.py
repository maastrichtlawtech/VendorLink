"""
Python version : 3.8
Description : Finetunes BERT with Adapter layers for Language Task on Dark-Web data. The prefix dark represents that the model
              has been pre-trained on the top of available checkpoint for the language task on Darknet market advertisements.

              The experiments showed that the performance of BERT-LM with adapter layers turned out to be worse than BERT-LM without
              it. Hence, for our further experiments, we decided not to use it for the classification task. 
"""
...
from transformers import (TrainingArguments, AdapterTrainer, set_seed)
...
parser = argparse.ArgumentParser(description="Finetuning BERT with Adapter layers for Language Task on Dark-Web data")
parser.add_argument('--mode', type=str, default="train", help="Can be train or evaluate")
parser.add_argument('--model', type=str, default="darkadapt-BERT", help="name of the model to be trained")
...
# add new adapter
model.add_adapter("adapt-dark-lm")
# activate adapter for training
model.train_adapter("adapt-dark-lm")
# Enable adapter training
model.set_active_adapters("adapt-dark-lm")
model.to(device)
...
trainer = AdapterTrainer(model=model,args=training_args, data_collator=data_collator, train_dataset=train_dataset, eval_dataset=eval_dataset)
...
if training_args.do_train:
    ...
    # Save model.
    trainer.save_model()
    model.save_all_adapters(args_parser.save_dir)
